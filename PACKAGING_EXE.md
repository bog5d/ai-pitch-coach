...
> **交付策略（必读）**  
> **推荐生产/同事分发**：使用根目录 `python build_release.py` 生成的 **BAT + 源码纯净包**（目录名为 **`AI路演教练_纯净交付版_{CURRENT_VERSION}`**，见脚本内变量），路径与子进程行为与开发机一致，排障成本最低。  
...
- **BAT 纯净包**：由 `python build_release.py` 生成 **`AI路演教练_纯净交付版_{版本}/`**（随 `CURRENT_VERSION` 变化），走 `pip install` + `streamlit run`，不含 `dist/`。
