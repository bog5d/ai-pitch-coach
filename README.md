...
| **V9.6.2 工业级稳定性十修** | 阿里云任务轮询 **GET**、长录音时间戳单位、**缓存命中跳过二次润色**、超长文件名路径保护、阶段一截断写入报告说明、错题 **`raw_text` 幂等**、指挥中心 **单次聚合读盘**、审查台草稿 **KeyError 防护**、逻辑冲突告警降噪等；回归结果以 `CHANGELOG.md` 最新条目为准。 |
...
| **V10.3 PM 红蓝对抗全项修复** | **P0** 短名称炸弹根治 + 备份机制 + 同步告警；**P1** 历史数据迁移 + 融资结果字段 + 记忆权重 90 天衰减；**P2** 会前演练模式（AI 扮投资人）+ 客户只读 Dashboard + 多页架构；**P3** 融资成功率预测 + Partner 投资人画像 + **多语言支持**（中英文自动检测，英文访谈全程英文输出）；回归结果以 `CHANGELOG.md` 最新条目为准。 |
| **V10.7.0 融资作战室 MVP** | 新增「🚀 融资作战室」左右分屏备战界面（倒计时、材料清单、历史弱点、DeepSeek 教练对话、语音输入、战后包扎），并完成红蓝对抗 P0~P3 加固（超时锁、发送锁、缓存 TTL、Prompt 消毒、错误脱敏、Windows 临时文件）。 |
...
生成目录：**`AI路演教练_纯净交付版_{版本}/`**（版本号由 `build_release.py` 的 `CURRENT_VERSION` 决定，当前为 **`AI路演教练_纯净交付版_V10.7.0/`**）— 可直接拷贝至 U 盘分发；同事双击 BAT 即可完成依赖安装与启动。
