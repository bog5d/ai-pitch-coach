...
| **V9.6.1 稳定性四连修** | ① 多音频批处理音频播放修复（存原始路径，不存临时 gateway）② 审查台翻车片段信息降载（4字段默认 + 专家视图折叠）③ 风险点数量约束（≤10个上限 + 质量门槛 Prompt）④ 空白卡片过滤（`_is_valid_risk_point` 守门）。228 passed，零新功能引入。 |
...
生成目录：**`AI路演教练_纯净交付版_{版本}/`**（版本号由 `build_release.py` 的 `CURRENT_VERSION` 决定，当前为 **`AI路演教练_纯净交付版_V9.6.1/`**）— 可直接拷贝至 U 盘分发；同事双击 BAT 即可完成依赖安装与启动。包内附带 **`V9.6.1_稳定性四连修版_说明.txt`**、`V9.6_…`、`V9.1_…` 等业务说明（若已列入白名单）。
