...
| # | 描述 | 位置 |
|---|------|------|
| Fix-1 | **投资人匹配结果展示 IndentationError**：`for i, r in enumerate(results)` 循环内 `with st.expander` 缩进 12 格导致 `col_s, col_d = st.columns(...)` 被放在 with 块外部，修正为标准 4 格递进缩进 | `app.py` 投资人匹配结果展示区块 |
