...
| **当前版本** | V9.6.5，全量 `pytest tests/` → **334 passed** |
| **本次目标** | V10.0 全部完成：共享记忆协议 + Analytics导出 + 飞轮看板 + 行业基准对比 |
| **执行原则** | 铁律一至五全部适用；TDD 优先；不触碰 schema.py；不引入云端依赖 |
| **整体进度** | ✅ Task1共享记忆；✅ Task2 Analytics导出；✅ Task3飞轮看板；✅ Task4行业基准引擎；✅ Task5/6/7文档收尾 |
...
## Task 4：行业基准引擎 + Dashboard 对比区块

> **执行状态（2026-04-11）**：✅ **完成** — 全部 Steps 通过；`pytest tests/` → **334 passed**。
> **测试状态**：`tests/test_v100_benchmark.py` **16/16 passed**。
> **已改文件**：`src/benchmark_engine.py`（新建）、`app.py`（import + 函数签名 + `_render_benchmark_section`）。

**目标**：扫描 workspace 下所有历史 `*_analytics.json`，聚合匿名行业基准（均分、分布、风险频率、精炼率、截断率），在机构画像 Dashboard 底部以卡片 + Plotly 柱状图呈现，实现**每家公司与行业基准的横向对比**。

**文件改动**：
- `src/benchmark_engine.py`（新建，约 90 行）：`scan_analytics_files` + `build_benchmark`
- `app.py`：`_v86_render_executive_dashboard` 签名加 `workspace_root` 参数；新增 `_render_benchmark_section` 函数；调用处透传 `workspace` 变量
- `tests/test_v100_benchmark.py`（新建，16 个测试）

**配套文档**：
- `.env.example`：补充 `MEMORY_ROOT` / `CACHE_ROOT` 详细注释和示例
- `ARCHITECTURE.md`：新增 8.5 节 V10.0 模块说明
- `小白保姆级操作手册.md`：新增"6️⃣ 多人共享网盘配置"章节

---

- [x] **Step 1：写失败测试（新建 tests/test_v100_benchmark.py）** ✅
- [x] **Step 2：实现 src/benchmark_engine.py** ✅
- [x] **Step 3：app.py Dashboard 加入行业基准对比区块** ✅
- [x] **Step 4：文档收尾（.env.example / ARCHITECTURE.md / 小白手册）** ✅
- [x] **Step 5：全量回归 334 passed** ✅
- [x] **Step 6：Commit** ✅

```bash
git commit -m "feat(V10.0-Task4): 行业基准引擎 + Dashboard对比看板 + 全部文档收尾"
```
